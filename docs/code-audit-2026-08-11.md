...
現時点で、テストと配布検査が通らない高優先度のコード不具合は確認していない。引用付きメディアの選択境界はコード修正済みで、残るのはXの現行DOMを使った動作確認である。FirefoxとGitHub Actionsの運用確認は優先度を下げ、ChromiumとX上の確認を先に進める。
...
| A-2 | 低 | 外部検証 | カスタムコードは両manifestへ登録され、ZIP検査も通るが、Firefox本体とログイン済みXの操作は未実施 | 使用予定が生じた時にFirefoxで列追加・再読込・メディア・リスト・投稿列を確認する |
...
| A-6 | 低 | ロケール拡張・コード修正済み | 有料パートナーシップ判定に日本語の`有料パートナーシップ`と英語の`Paid partnership`を追加し、回帰テストで確認した | 表示文言以外の安定属性が提供された場合は、文言依存をさらに減らす |
| A-7 | 低 | 設計保留 | タブ状態は現在、追加・削除・並べ替え時の位置remapで保護している | 全プロファイル移行、旧データ互換、実ブラウザーfixtureを揃えてから安定ID化を検討する |
| A-8 | 低 | 外部運用 | `sync-upstream.yml`のレビューIssue作成・更新はGitHub上の手動dispatchを必要とする | 本家更新を継続的に追跡する必要が生じた時に、更新なし／更新ありの2ケースを一度だけ実行確認する |
