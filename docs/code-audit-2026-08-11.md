...
現時点で、テストと配布検査が通らない高優先度のコード不具合は確認していない。残る項目は、Xの現行DOM・ログイン状態・Firefox本体・GitHub Actionsなど、ローカルの静的検証だけでは確定できないものと、利用条件が揃った時に改善する低優先度の設計課題に分ける。

## 検証結果

- `node tests/run.mjs`: 32/32 成功
- `verify.ps1`: 成功
...
| --- | --- | --- | --- | --- |
| A-1 | 中 | 未検証の不具合候補 | `extensions/media_viewer_block_helper.js`に、引用内メディアのクリックで元投稿側のメディアが選ばれる可能性を示すTODOが残る | 現行Xの引用DOMとReact propsを保存したfixtureを作り、引用元・引用先の選択規則を実ブラウザーで確認する |
