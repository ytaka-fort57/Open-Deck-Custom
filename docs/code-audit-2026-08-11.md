...
現時点で、テストと配布検査が通らない高優先度のコード不具合は確認していない。引用付きメディアの選択境界はコード修正済みで、残るのはXの現行DOMを使った動作確認である。Firefoxは使用予定がないため優先度を下げ、ChromiumとX上の確認、およびGitHub Actionsの運用確認を先に進める。
