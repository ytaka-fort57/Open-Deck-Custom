...
| R-2 | 高 | 設定モデルとDOM変換 | 設定からHTMLを作る処理と、DOMから設定を保存する処理が離れており、4種類のカラム追加処理も同じplaceholder置換を重複している | `column_settings`の正規化、DOM属性への変換、DOMからの読み取りを一つの境界へ集約する。設定のround-tripテストを追加する |
...
| R-8 | 中 | 実DOMテスト | `content.js`の回帰検査は配線やソース文字列の検査が中心で、巨大なDOM初期化を直接実行していない | DOM fixtureを追加し、カラム追加・削除・再構築・並び替え・iframe loadを実行検証する。分割作業の前提にする |
...
1. R-8のDOM fixtureを追加し、既存の63件を維持する。
2. R-2の設定モデルとカラムrendererを抽出する。
