...
本プロジェクトの実運用・手動確認対象は Microsoft Edge（Chromium）です。Firefox は使用予定がなく、
Firefox 版の実機起動・操作確認は実施しません。既存の `manifest_firefox.json` と Firefox ZIP に対する
JSON・参照ファイル・許可リストの自動検査だけは、配布定義の破損検出のために継続します。
