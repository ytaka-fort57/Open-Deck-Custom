...
擬似popstate方式は、Xのルーターが再描画してくれることに依存する。原因1〜5を直しても
「Xが受け取ったstateを無視して何もしない」場合は残り、そのときは戻れない。

そこで、戻る操作の成否を見て段階的に手段を上げる。

1. `replaceState` + 擬似popstate（従来）。成功すれば再読み込みが無く、スクロール位置も残る
2. 800ms（`BACK_FALLBACK_MS`）待って描画が変わらなければ、戻り先URLを**実際に読み込む**
   （`navigate_to_target`）。SPA遷移ではないため必ず戻り先が描画される
3. カラムの文書を参照できず実ナビゲーションもできない場合だけ、従来どおりURLを巻き戻して
   次のBackspaceへ委ねる（`rollback`）

実ナビゲーションには `location.reload()`（URLが既に戻り先の場合）または
`location.replace()` を使う。`location.assign()` は使わない。joint session historyへ
エントリが積まれ、そのカラムが「直近に遷移したフレーム」になってX標準の戻るを狂わせる
（原因4と同じ機序）。reload / replace はどちらも現在のエントリを置き換えるため安全である。

実ナビゲーションで戻した後は、描画の手掛かり（`render_signal`）を見ない。読み込み結果が
戻り先そのものなので、比較する必要が無い。これにより「戻る前後で title・aria-label・
先頭記事がすべて同じ画面」でも戻れるようになり、`render_signal` の弱点が実害を持たなくなる。

代償は、この経路を通ったときだけカラムが再読み込みされ、スクロール位置が失われることである。
通常はステップ1で完了するため、この経路には落ちない。

なお読み込み中はカラムのURLを参照できない。その間に戻る待ちが解けないままになると、
そのカラムのBackspaceが以後効かなくなるため、15秒（`BACK_ABANDON_TIMEOUT_MS`）参照できない
場合は待ちを解く。
...
- 実ナビゲーションで戻した後の成否を `render_signal` で判定しない。読み込み結果が
  戻り先そのものであり、手掛かりが一致しても戻れている。
...
- `tests/column_history.test.mjs`
  ...
  実ナビゲーションができない場合のURL巻き戻しと履歴保持、読み込みが長引いた場合に
  戻る待ちが解けることも併せて固定する
