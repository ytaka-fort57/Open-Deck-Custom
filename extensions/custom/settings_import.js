...
window.addEventListener("load", function(){
    ...
    function save_import_data(import_data){
        chrome.storage.local.get("opd_settings", function(value){
            try{
                ...
                chrome.storage.local.set(storage_items, function(){
                    if(chrome.runtime.lastError != null){
                        set_status("インポートできません: " + chrome.runtime.lastError.message, true);
                        return;
                    }
    ...
    document.getElementById("export_btn").addEventListener("click", function(){
        chrome.storage.local.get(["opd_settings", "opd_profile_store", "opd_custom_column_state"], function(value){
            if(chrome.runtime.lastError != null){
                set_status("保存済み設定を読み出せません: " + chrome.runtime.lastError.message, true);
                return;
            }
